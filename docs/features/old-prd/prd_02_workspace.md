...
- 版本说明：
  - V9.5 Final —— 确立“服务端驱动（Server-Driven）”交互原则；明确追问建议与图表渲染均由 Agent 工作流动态决策，移除前端硬逻辑；整合 UI 视觉规范。
  - [vNext] Addendum —— 对齐 AI Agent OS 设计趋势：补齐 Semantic Zoom（L1/L2/L3）、Process Debug Panel、Context Budget 可视化、Channel 协作与 Live Canvas（编辑器/工区）联动。
- 相关文档（Links）：
  - 架构设计：<docs/architecture/...>
  - API 契约：<docs/api/...>
  - 数据模型：<docs/data/...>
  - 埋点/指标口径：<docs/data/metrics.md#workspace>
  - 审计与日志：<docs/data/audit.md#workspace>
  - 测试与验收：<docs/quality/...>
...
- 最大风险与依赖：
  ...
  - VFS 写入/补丁与编辑器锁需要与权限（IA）与审计（Syscall/Tool/Trace）对齐。
...
> 指标口径应在 `docs/data/metrics.md#workspace` 统一定义；本节只给出推荐字段与最小可计算定义。

- 北极星指标（建议）：**关键任务完成率**
  - 定义：`task_completed = process_completed AND (editor_export OR copy_formatted OR team_share)`
  - 口径：`task_completed / process_started`
- 关键指标（建议）：
  - 首字延迟 < 1s 达成率：`first_token_lt_1s / process_started`
  - 编辑器打开率：`editor_open / deliverable_prompt_shown`
  - 导出率：`editor_export / editor_open`
  - suggestion_chips 展示率与点击率：`chips_shown / msg_final`，`chips_click / chips_shown`
  - 临时对话启用率：`chat_temp_mode_on / chat_new`
- 护栏指标（建议）：
  - Server-Driven UI 渲染失败率：`sdui_render_failed / sdui_render_attempt`
  - 文件上传失败率：`file_upload_failed / file_upload_attempt`
  - 断线重连失败率：`reconnect_failed / reconnect_attempt`
...
> 目标：把“产品全景结论”写进 PRD，但通过 Semantic Zoom（L1/L2/L3）与 Phase（Now/Next/Later）锁范围。
> - Now：不破坏 MVP；补齐 UI Schema 版本化、Debug 可视化入口、Context/Trace 的最小字段回传。
> - Next：落地可用的 Process Debug Panel、Context Budget 可视化、Channel 协作与 Live Canvas 联动。
> - Later：实现 IDE 级可观测与多人协作（Spectate/Intervene）、以及可编排的 UI Component Marketplace。

### 11.1 vNext 能力全景（Workspace 视角 Capability Map）

| Capability | 用户价值 | Phase | 依赖 | 最小验收（示例） |
|---|---|---|---|---|
| Semantic Zoom（L1/L2/L3） | 同一任务“摘要→结构→调试”渐进呈现 | Now | Agent Bridge / UI Runtime | 支持 L1（简报）、L2（结构组件）、L3（调试信息）三档切换 |
| Process Debug Panel | 失败可解释、可介入、可追责 | Next | Trace/Tool Audit（IA-012） | 任一失败可看到：最近 N 个 span、关键输入输出 hash、policy 决策 |
| Context Budget 可视化 | 成本/安全可控（上下文注入可见） | Next | IA-011 / Runtime | 展示 used/max、超限策略与触发原因 |
| Channels（围观/插话/转交） | 多人协作与“人机共在” | Later | IA-013 / Realtime | 会话支持 spectate/intervene 权限与动作审计 |
| Live Canvas（交付物工作流） | 交付物成为“工程资产”而非消息 | Next | Editor/VFS/Locks | 编辑器支持任务绑定、版本快照与可回滚 |

### 11.2 vNext 需求总览表

| Req ID | 标题 | 优先级 | Phase | 依赖 |
|---|---|---|---|---|
| WS-014 | Semantic Zoom：L1/L2/L3 视图切换与权限 | P1 | Now | UI Runtime / IA |
| WS-015 | Server-Driven UI Schema 版本化 + 组件白名单 | P0 | Now | UI Schema / Sandbox |
| WS-016 | Process Debug Panel：Trace/Span/Tool 可视化与介入 | P1 | Next | IA-012 / Telemetry |
| WS-017 | Context Budget Panel：配额/负载/超限策略展示 | P1 | Next | IA-011 |
| WS-018 | Channels：Spectate/Intervene/Hand-off 协作流 | P2 | Later | IA-013 / Realtime |
| WS-019 | Live Canvas：交付物绑定任务、版本回滚、多人锁 | P1 | Next | Editor/VFS/Locks |
...
| Semantic Zoom | 同一任务从“摘要→结构→调试”的渐进呈现方式（L1/L2/L3） |
| Process Debug Panel | 面向诊断与治理的调试面板：trace/span/tool/policy 时间线 |
