...
## 补充（不改变以上任何原文，用于后续迭代对齐）

> 本节为新增内容，不替代、不改写、不穿插修改上方任何原文条目；仅用于把“可落地实现”所需的系统对齐点一次性补齐。

### A. Semantic Zoom：全景与先做什么

- 全景能力：ROI 指标体系（M1~M5）+ 明细导出 + 约束（<15min / 180d）+ 口径治理（标准工时/取消/失败）+ 审计
- 先做顺序（建议）：
  1) IN-007/IN-008 导出闭环（字段完整、范围正确、下载稳定）
  2) IN-009 数据留存与延迟可观测（延迟指标、ETL 监控、TTL）
  3) M1/M4 基础可信指标（Completed/Success Rate）
  4) M2/M3 价值与质量（Hours Saved / Intervention）
  5) M5 自治等级（若需，明确来源：配置或运行时推断）

### B. 与 Workspace / Marketplace 的对齐点清单（仅补充）

- Workspace：
  - Regenerate 事件、编辑器修改事件需提供可去重的 `task_id/process_id` 关联键。
  - “用户主动取消”需要独立事件与状态码，供 IN-005/IN-005-1 口径排除。
- Marketplace：
  - “标准工时（小时/次）”建议作为 Workflow/Agent 的可配置字段（默认值 0.1），并提供版本化与审计（谁改、何时改）。

### C. 数据口径落地建议（仅补充）

- 建议在 `docs/data/metrics.md` 建立章节：
  - Tasks Completed：过滤规则（试错/报错会话如何识别）
  - Task Success Rate：排除取消的定义与实现
  - Human Intervention Rate：事件来源、计数窗口、去重规则
  - Estimated Hours Saved：标准工时来源、默认值、FTE 换算常量（160h/月）
